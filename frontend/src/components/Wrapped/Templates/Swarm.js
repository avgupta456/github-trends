...
const SwarmPlot = ({ data, type }) => {
  const currData = data.contribs;
      ...
      {Array.isArray(currData) && currData.length > 0 && (
        <>
          <p className="text-xl font-semibold">
            {type === 'type'
              ? 'Contributions by Time and Type'
              : 'Contributions by Time and Day'}
          </p>
          <p>{`${currData.length} Sampled Contributions`}</p>
          <ResponsiveSwarmPlot
            theme={theme}
            isInteractive={false}
            data={currData}
            ...
            axisLeft={{
              ...
              format: (value) => {
                ...
                const minutes = String(
                  Math.floor((value % 3600) / 60 / 10) * 10,
                );
